...
- __Ubuntu/Debian:__ `sudo apt-get install curl libarchive-tools cpio gzip openssl`
- __Fedora:__ `dnf install curl bsdtar cpio gzip openssl`
- __(untested) Arch Linux:__ `sudo pacman -Syu curl bsdtar cpio gzip openssl`
...
2. Depending on the type of link you find, follow the appropriate steps:
    - __For google drive links:__ The url looks something like https://drive.google.com/file/d/1O61pmNRqaSBbFo8QGJFIPBqOagtX5E8x/view?usp=share_link. Copy the random letters part between `d/` and `/view` (in this case `1O61pmNRqaSBbFo8QGJFIPBqOagtX5E8x`) and put it in `version.yml` in the `gdoc_fileid` field. Also change `download_type` to `gdrive` in `version.yml`.
    - __For mega.nz links:__ paste the url in `version.yml` in the `mega_url` field. Also change `download_type` to `mega` in `version.yml`.
    - __For direct download links:__ paste the url in `version.yml` in the `direct_url` field. Also change `download_type` to `direct` in `version.yml`.
3. (Optional) While your'e in `version.yml`, update the `version` field to the new version name.
